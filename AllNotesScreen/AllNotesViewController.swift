...
class AllNotesViewController: UIViewController{
    ...
    var coordinator: AllNotesCoordinator?
    ...
    @IBAction func addNoteAction(_ sender: UIButton) {
        coordinator?.gotoAddNote(self, self)
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !editToolbar.isHidden {
            ...
            if !tblAllNotes.allowsMultipleSelection {
                mainView.alpha = 0.5
                coordinator?.gotoViewNote(self,paginationNotes[indexPath.row])
            }
        }
    }
